...
contains
    ...
    subroutine HMC_Phys_LSM_Cpl(iID, iRows, iCols, sTime)
        ...
        integer(kind = 4)           :: iFlagDynVeg, iFlagEnergyBalance
        ...
        real(kind = 4), dimension(iRows, iCols)     :: a2dVarLSTPStep, a2dVarLSTUpd, a2dVarLSTTmp, a2dVarETUpd, a2dVarETpotUpd
        
        real(kind = 4), dimension(iRows, iCols)     :: a2dVarTDeep, a2dVarBF, a2dVarCH, a2dVarEpsS
        real(kind = 4), dimension(iRows, iCols)     :: a2dVarBF_BareSoil, a2dVarBareSoil 
        ...
        a2dVarET = 0.0; a2dVarETpot = 0.0; a2dVarETUpd = 0.0; a2dVarETpotUpd = 0.0;
        a2dVarEF = 0.0;
        a2dVarSnowMask = 0.0
        ...
        !-------------------------------------------------------------------------------------
        ! Flag to activate energy balance or use data provided by forcing datasets
        if (iFlagEnergyBalance .eq. 1) then
        
            !-------------------------------------------------------------------------------------
            ! Info module activation
            ...
            !-------------------------------------------------------------------------------------
            ! Info module activation
            call mprintf(.true., iINFO_Verbose, ' Phys :: Land surface model :: DEACTIVATED' )
            
            ! Nullify variables when LSM is deactivated
            a2dVarRn = -9999.0
            a2dVarH = -9999.0
            a2dVarLE = -9999.0
            a2dVarG = -9999.0
        ...
        if (iDEBUG.gt.0) then
            ...
            call mprintf(.true., iINFO_Verbose, checkvar(a2dVarETpotUpd, a2iVarMask, 'ET.POT END ') )
	...
	oHMC_Vars(iID)%a2dET = a2dVarETUpd
        oHMC_Vars(iID)%a2dETpot = a2dVarETpotUpd
